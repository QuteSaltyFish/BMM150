...
int8_t BMM150::begin_i2c(uint8_t i2c_addr = BMM150_CHIP_ID_ADDR,
  ...
  if (!i2c_dev->begin()) {
    return BMM150_E_NOT_INITIALIZED;
  }

  return BMM150_OK;
}

int8_t BMM150::begin_spi(uint8_t cs_pin, SPIClass* theSPI) {
  /* Power up the sensor from suspend to sleep mode */
  set_op_mode(BMM150_SLEEP_MODE);
  delay(BMM150_START_UP_TIME);
...
void BMM150::read_mag_data() {
  ...
  Adafruit_BusIO_Register data_reg =
      Adafruit_BusIO_Register(i2c_dev, spi_dev, ADDRBIT8_HIGH_TOREAD,
                              BMM150_DATA_X_LSB, BMM150_XYZR_DATA_LEN);
  int8_t reg_data[BMM150_XYZR_DATA_LEN];
  // TODO: Test the implementation
  data_reg.read((uint8_t*)reg_data, BMM150_XYZR_DATA_LEN);
